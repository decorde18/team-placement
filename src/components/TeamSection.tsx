...
import { Player, TeamConfig, PlayerStatus, SortOption, FilterOption, PlayerAttendance } from '@/lib/mockData';
import { PlayerCard } from './PlayerCard';
import { Users, Trash2, ArrowUpDown, Filter } from 'lucide-react';
    ...
    <div className="flex flex-wrap items-center gap-2 mt-2 mb-3">
      <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-md px-2 py-1 shadow-sm">
        ...
        >
          ...
          {isTryoutMode && <option value="status">Sort by Status</option>}
        </select>
      </div>
      ...
      {team.id !== 'unassigned' && isTryoutMode && (
        <button 
          onClick={() => onInviteAllTeam?.(team.id)}
          className="ml-auto text-xs font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded-md transition-colors"
        >
          Invite All
        </button>
      )}
      ...
      {team.id !== 'unassigned' && (
        <button 
          ...
          className={cn(
            "text-gray-400 hover:text-red-500 hover:bg-red-50 p-1 rounded-md transition-colors",
            !isTryoutMode ? "ml-auto" : ""
          )}
          title="Delete Team"
        >
          <Trash2 size={16} />
